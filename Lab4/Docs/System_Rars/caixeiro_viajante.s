...
SORTEIO:
	...
	LOOP_SORTEIO:
		...
		ecall		
		li t0,310
		remu a0,a0,t0 # faz numero mod 310 para obter a coordenada X
		sw a0,0(s1) # armazena coord X em C
		
		li a7,41 # gera numero aleatorio para Y, em a0
		ecall
		li t0,230
		remu a0,a0,t0 # faz numero mod 230 para obter a coordenada Y
...
ORDENA:
	...
	TESTA_ROTA:
		...
		LOOP_SOMADIST: # loop para somar distancias
			...
			mv t1,s2
			rem t2,s4,s0 # t2 = i mod n
			addi t3,s4,1 # t3 = i+1
			rem t3,t3,s0 # t3 = (i+1) mod n
			la t4,vet_permut
			slli t2,t2,2 # t2 * 4 bytes
			add t2,t4,t2 # a[i mod n]
			slli t3,t3,2 # t3 * 4 bytes
			add t3,t4,t3 # a[(i+1 mod n]
			lw t2,0(t2)
			lw t3,0(t3)
